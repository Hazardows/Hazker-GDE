...
b8 vk_create_graphics_pipeline(
    ...
    HzeroMemory(&color_blend_attachment_state, sizeof(VkPipelineColorBlendAttachmentState));
    ...
    pipeline_create_info.layout = out_pipeline->pipelineLayout;
    ...
    HERROR("vkCreateGraphicsPipelines failed with %s.", vkr_toString(result, true));
    return false;
}

void vk_destroy_graphics_pipeline(vulkanContext* context, vulkanPipeline* pipeline) {
    if (pipeline) {
        // Destroy pipeline
        if (pipeline->handle) {
        ...
        if (pipeline->pipelineLayout) {
            vkDestroyPipelineLayout(context->device.logical_device, pipeline->pipelineLayout, context->allocator);
            pipeline->pipelineLayout = NULL;
        }
    }
}

void vk_bind_pipeline(vulkanCommandBuffer* command_buffer, VkPipelineBindPoint bind_point, vulkanPipeline* pipeline) {
