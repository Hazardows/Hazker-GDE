...
u8 linear_allocator_single_allocation_all_space() {
    ...
    // Single allocation.
    void* block = allocate_linear_allocator(&alloc, sizeof(u64));
...
u8 linear_allocator_multi_allocation_all_space() {
    u64 max_allocs = 1024;
    linear_allocator alloc;
    create_linear_allocator(sizeof(u64) * max_allocs, 0, &alloc);

    // Multiple allocations - full.
    void* block;
    for (u64 i = 0; i < max_allocs; i++) {
        block = allocate_linear_allocator(&alloc, sizeof(u64));
        // Validate it
        expect_should_not_be(0, block);
